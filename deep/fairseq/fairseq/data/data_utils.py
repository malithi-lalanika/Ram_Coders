...
def get_pad_size(values, input_shapes, pad_to_length=None):
    ...
    #print('values1:',len(values[0]))
    for batch_size, padlen in [[512, 8], [256, 16], [128, 32]]:
        #print('batch',batch_size)
        #if len(values[0]) == batch_size:
...
def load_indexed_dataset(path, dictionary=None, dataset_impl=None, combine=False, default='cached'):
    ...
    for k in itertools.count():
        ...
        if dataset is not None:
            print('[JJ] finished loading data_129', len(dataset))
        if dataset is None:
            break
...
def load_shard_indexed_dataset(path, dictionary=None, dataset_impl=None, combine=False, default='cached'):
    ...
    if dataset is not None:
        print('[JJ] finished loading data_178', len(dataset))
...
def batch_by_size(
    ...
    if fixed_shapes is None:
        ...
        print("fixed_shape: ",fixed_shapes)
        fixed_shapes= np.array([[512, 8], [256, 16], [128, 32]], dtype = np.int64)
