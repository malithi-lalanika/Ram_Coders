...
class BaseFairseqModel(nn.Module):
    ...
    def load_state_dict(
        ...
        ############for paradise finetune
        encoder_layer_norm_weight = torch.ones(1024)
        state_dict['encoder.layer_norm.weight'] = encoder_layer_norm_weight

        # Set encoder.layer_norm.bias in the state_dict
        encoder_layer_norm_bias = torch.zeros(1024)
        state_dict['encoder.layer_norm.bias'] = encoder_layer_norm_bias

        # Set decoder.layer_norm.weight in the state_dict
        decoder_layer_norm_weight = torch.ones(1024)
        state_dict['decoder.layer_norm.weight'] = decoder_layer_norm_weight

        # Set decoder.layer_norm.bias in the state_dict
        decoder_layer_norm_bias = torch.zeros(1024)
        state_dict['decoder.layer_norm.bias'] = decoder_layer_norm_bias
        ############for paradise finetune
